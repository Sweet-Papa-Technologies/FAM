...
import { generateRooCodeConfig } from '../../../src/generators/roo-code.js'
...
function makeInput(overrides?: Partial<GeneratorInput>): GeneratorInput {
  return {
    ...
    sessionToken: 'fam_sk_roo_a1b2c3d4',
    daemonUrl: 'http://localhost:7865',
    ...overrides,
  }
}

describe('generateRooCodeConfig', () => {
  ...
  it('should default to project-level .roo/mcp.json path', () => {
    const result = generateRooCodeConfig(makeInput())
    expect(result.path).toBe('.roo/mcp.json')
  })
  ...
  it('should handle daemon URL with trailing slash', () => {
    const result = generateRooCodeConfig(
